#include <string>
#include <queue>
#include <vector>
#include <iostream>
#include <iomanip>

class MazeMaker {
public:
	int longestPath(std::vector<std::string> maze, int startRow, int startCol,
					std::vector<int> moveRow, std::vector<int> moveCol) {

		int max = 0;
		int width = maze[0].size();
		int height = maze.size();
		int board[50][50];
		int movePatternNum = moveRow.size();
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				board[i][j] = -1;
			}
		}

		board[startRow][startCol] = 0;
		std::queue<int> queueX;
		std::queue<int> queueY;
		queueX.push(startCol);
		queueY.push(startRow);

		while (queueX.size() > 0) {
			int x = queueX.front();
			int y = queueY.front();

			queueX.pop();
			queueY.pop();

			for (int i = 0; i < movePatternNum; i++) {
				int nextX = x + moveCol[i];
				int nextY = y + moveRow[i];

				if (0 > nextX || nextX >= width) continue;
				if (0 > nextY || nextY >= height) continue;
				if (board[nextY][nextX] != -1) continue;

				if (maze[nextY][nextX] == '.') {
					board[nextY][nextX] = board[y][x] + 1;
					queueX.push(nextX);
					queueY.push(nextY);
				}
			}
		}

		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				if (maze[i][j] == '.' && board[i][j] == -1) {
					return -1;
				}
				max = std::max(max, board[i][j]);
			}
		}

		return max;
	}
};

